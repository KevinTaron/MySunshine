...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements
        ForecastFragment.Callback, DataApi.DataListener,
...
//    private static final long DAY_IN_MILLIS = 1000 * 60;
    ...
    private void updateWearable(Double low, Double high, String desc) {
        Log.d("test", "low temp: " + low.toString());
        Log.d("test", "high temp: " + high.toString());
        Log.d("test", "desc: " + desc);

        int highi = (int) Math.round(high);
        int lowi = (int) Math.round(low);

        PutDataMapRequest dataMap = PutDataMapRequest.create(WEATHER_PATH);
        dataMap.getDataMap().putString("maxTemp", Integer.toString(highi));
        dataMap.getDataMap().putString("minTemp", Integer.toString(lowi));
        dataMap.getDataMap().putString("icon", desc);
        ...
        Wearable.DataApi.putDataItem(mGoogleApiClient, request)
                .setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
                    ...
                    public void onResult(DataApi.DataItemResult dataItemResult) {
                        Log.d(LOG_TAG, "Sending weather was successful: " + dataItemResult.getStatus()
                                .isSuccess());
                    }
                });
    }
    ...
    private void updateWearable() {
        ...
        if (cursor.moveToFirst()) {
            ...
            Log.d("test", "low temp: " + Integer.toString(highi));
            Log.d("test", "high temp: " + Integer.toString(lowi));
            Log.d("test", "desc: " + desc);
